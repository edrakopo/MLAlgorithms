...
def optimise_model(model, alg_name, search_method, params):
    ...
    if search_method=="random":
       #---- RandomizedSearchCV ----
       random_search = RandomizedSearchCV(model, param_distributions=params, n_iter=param_comb, scoring='roc_auc', n_jobs=4, cv=skf.split(X_train, y_train), verbose=3, random_state=101 )
    ...
    random_search.fit(X_train, y_train, eval_metric=["error",], eval_set=eval_set, early_stopping_rounds=5) #using the estimator’s score method
