...
void tankCreatecsv()
{
   ...
   ofstream csvfile;
    ...
    csvfile.open ("tankPMT_forEnergy_bigfile.csv");
...
*/
   ...
   sprintf(fname, "/home/tinabel/Desktop/ANNIE_VertexReco/vtxreco/vtxreco-beamlikemrd.root");
   ...
   Int_t run, event, trigger, recoStatus, fNdigitLAPPDs, fNdigitPMTs;
  ...
  // regTree->SetBranchAddress("recoDirZ", &dirZ);
   ...
   regTree->SetBranchAddress("NDigitsPMTs",&NDigitsPMTs);
   regTree->SetBranchAddress("NDigitsLAPPDs",&NDigitsLAPPDs);
...
//for (Long64_t ievt=0; ievt<10; ievt++) {
 for(Long64_t ievt=0; ievt<regTree->GetEntries(); ievt++) {
   ...
   nhits = NDigitsLAPPDs + NDigitsPMTs + 9999*2;
 	//if(recoVtxFOM<=0){
      	...
      	if(-50<truevtxX && truevtxX<=50){ 
      		if(-50<truevtxY && truevtxY<=50){ 
				...
				for(int k=0; k<nhits; k++){
				  ...
				  //NdigitPMTs[k]=NdigitPMTs->at(k)
				  //NdigitLAPPDs[k]=NdigitLAPPDs->at(k)
