...
#infile = "Etrue_Ereco.csv"
infile = "Etrue_Ereco_after_phase_till1000.csv"

filein = open(str(infile))
print("number of events: ",filein)
df00=pd.read_csv(filein)
print("df00.head() ",df00.head())
Etrue = df00['TrueEnergy']
Ereco = df00['DNNEnergy']
...
#plt.show()
fig.savefig('Etrue_Erecoscatter_till1000.png') 
plt.close(fig)
...
#cmap = sns.light_palette('b',as_cmap=True) 
     ...
     plt.title(title)
     plt.show()
     fig0.savefig('DE_E_feat_till1000.png')   # save the figure to file
     plt.close(fig0)
...
ax1.legend(('MC','Reco'))
plt.show()
fig1.savefig('resol_Energy_till1000.png')   # save the figure to file
plt.close(fig1)
